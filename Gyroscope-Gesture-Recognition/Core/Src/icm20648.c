 ...
 ******************************************************************************/
uint32_t ICM20648_init(I2C_HandleTypeDef * handle)
{
  uint8_t data;

  /* Enable and setup the SPI bus */
  ICM20648_i2cInit(handle);
 ...
 *    for the x, y and z axes in g units. I have converted to ms^-2.
 *
 * @return
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20648_accelDataRead(float *accel)
{
  ...
  accel[0] = (float) temp * accelRes * 9.81;
  temp = ( (int16_t) rawData[2] << 8) | rawData[3];
  accel[1] = (float) temp * accelRes * 9.81;
  temp = ( (int16_t) rawData[4] << 8) | rawData[5];
  accel[2] = (float) temp * accelRes * 9.81;
 ...
 *    for the x, y and z axes in deg/sec units. Converted to rads^-1.
 *
 * @return
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20648_gyroDataRead(float *gyro)
{
  ...
  temp = ( (int16_t) rawData[0] << 8) | rawData[1];
  gyro[0] = (float) temp * gyroRes * (pi/180.0);
  temp = ( (int16_t) rawData[2] << 8) | rawData[3];
  gyro[1] = (float) temp * gyroRes * (pi/180.0);
  temp = ( (int16_t) rawData[4] << 8) | rawData[5];
  gyro[2] = (float) temp * gyroRes * (pi/180.0);
...
uint32_t ICM20648_accelGyroCalibrate(float *accelBiasScaled, float *gyroBiasScaled)
{
  ...
  {
	  ICM20648_registerRead(ICM20648_REG_XA_OFFSET_H, 2, &data[0]);
	  accelBiasFactory[0] = ( (int16_t) (data[0] << 7) | (data[1] >> 1));
	  ICM20648_registerRead(ICM20648_REG_YA_OFFSET_H, 2, &data[0]);
	  accelBiasFactory[1] = ( (int16_t) (data[0] << 7) | (data[1] >> 1));
	  ICM20648_registerRead(ICM20648_REG_ZA_OFFSET_H, 2, &data[0]);
	  accelBiasFactory[2] = ( (int16_t) (data[0] << 7) | (data[1] >> 1));
  }
  ...
  {
	  accelBiasFactory[0] -= (accelBias[0] / 8);
	  accelBiasFactory[1] -= (accelBias[1] / 8);
	  accelBiasFactory[2] -= (accelBias[2] / 8);
  }
...
//  /* Split the values into two bytes */
  ...
  {
	  uint8_t temp;
	  data[0] = (accelBiasFactory[0] >> 7) & 0xFF;
	  temp = data[1];
	  data[1] = ((accelBiasFactory[0] << 1) & 0xFE) | (temp & 1);
	  data[2] = (accelBiasFactory[1] >> 7) & 0xFF;
	  temp = data[3];
	  data[3] = ((accelBiasFactory[1] << 1) & 0xFE) | (temp & 1);
	  data[4] = (accelBiasFactory[2] >> 7) & 0xFF;
	  temp = data[5];
	  data[5] = ((accelBiasFactory[2] << 1) & 0xFE) | (temp & 1);
  }
...
/***************************************************************************//**
 * @brief
 *    Initializes the I2C bus in order to communicate with the ICM20648
 *
 * @return
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20648_i2cInit(I2C_HandleTypeDef * handle)
{
	hi2c = handle;
...
void ICM20648_registerRead(uint16_t addr, int numBytes, uint8_t *data)
{
  ...
  bank = (uint8_t) ((addr >> 7) << 4);

  //ICM20648_bankSelect(bank);

  HAL_I2C_Mem_Write(hi2c,208,ICM20648_REG_BANK_SEL,(uint16_t)1,&bank,1,HAL_MAX_DELAY);
  HAL_I2C_Mem_Read(hi2c,208,(uint16_t)regAddr,(uint16_t)1,data,numBytes,HAL_MAX_DELAY);
...
void ICM20648_registerWrite(uint16_t addr, uint8_t data)
{
  ...
  HAL_I2C_Mem_Write(hi2c,208,ICM20648_REG_BANK_SEL,(uint16_t)1,&bank,1,HAL_MAX_DELAY);
  HAL_I2C_Mem_Write(hi2c,208,(uint16_t)regAddr,(uint16_t)1,&data,1,HAL_MAX_DELAY);
...
{
	uint8_t newbank = bank << 4;
	HAL_I2C_Mem_Write(hi2c,208,ICM20648_REG_BANK_SEL,(uint16_t)1,&newbank,1,HAL_MAX_DELAY);
  /* Enable chip select */
  // ICM20648_chipSelectSet(true);
